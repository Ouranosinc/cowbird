...
def generate_redirects(app):
    ...
    But then, rendering on GitHub becomes this raw string with an hyperlink reference that doesn't lead to the
    ...
    In other words, when ``file1.rst`` is viewed from GitHub, the literal relative path is used and the file is found,
    ...
    if not isinstance(app.builder, builders.StandaloneHTMLBuilder):
        ...
        LOGGER.warning(f"Extension '{ext}' is only supported by the 'html' builder. Skipping...")
        return
    ...
    for from_path, to_path in app.config.doc_redirect_map.items():
        LOGGER.debug(f"Redirecting [{from_path}] -> [{to_path}]")

        rst_path = from_path
        if not rst_path.endswith(in_suffix) and not os.path.splitext(rst_path)[-1]:
        ...
        LOGGER.debug(f"RST  [{rst_path}] -> [{to_path}]")
        LOGGER.debug(f"HTML [{html_path}] -> [{to_path}]")
        ...
        if not os.path.exists(redirected_html_file):
            ...
            with open(redirected_html_file, "w", encoding="utf-8") as f:
