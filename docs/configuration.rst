...
The ``sync_permissions`` are defined first by a list of Magpie services and their associated resources,
defined in the ``services`` section below. The mappings defining how the resources should be synchronized are described
in the other section ``permissions_mapping``.

services:
_________

This section contains the different resources that can be synchronized, ordered by service. The services found in
this section of the config should also exist in Magpie.
...
They should correspond to the names used in the ``permissions_mapping`` section below.
